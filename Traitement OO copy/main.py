...
class Main:
    ...
    def __creationDroites(self):
        for point in self.getPoints():
            ...
            for droite in self.getDroites():
                bdroite=droite.getRegLin()[1]
                (apoint, bpoint) = point.getRegLin()
                if (abs(bpoint-bdroite)<700 and 
                    abs(droite.compareAnglePoint(apoint)) < 30 and
                    place==0):
    ...
    def PlotDroitesMoy(self):
        for droite in self.getDroites():
            if len(droite.getPoints())>20:
                (a,b)=droite.getRegLin()
                self.PlotDroiteMoy(a, b)
    ...
    def PlotDroites(self):
        ...
        i=0
        for droite in self.getDroites():
            if len(droite.getPoints())>10:
                color=colors[i%len(colors)]
                i+=1
                self.PlotPoints(droite.getPoints(), color+'o', text='no')
                (a,b)=droite.getRegLin()
                self.PlotDroiteMoy(a, b , color+'-')

    def PrintPoints(self):
        points=[]
        for point in self.getPoints():
            points.append([point.getXLisse()+4000, point.getYLisse()+5000])
        print(points)
